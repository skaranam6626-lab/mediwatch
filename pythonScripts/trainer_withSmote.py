...
logger=logging.getLogger('trainer_withSmote')
...
def loadAndPrepare(dataPath: str) -> tuple:
    ...
    ### Perform Imputation and SMOTE as data is imbalanced
    ...
    #Perform MinMax Scaling to normalize the data
    #minMaxScaler=MinMaxScaler()
    #xtrainScaled=minMaxScaler.fit_transform(xtrainSmote)
    #xtestScaled=minMaxScaler.transform(xtest)
    
    x_train_smote_df = pd.DataFrame(xtrainSmote, columns=X.columns)

    return x_train_smote_df,xtest,ytrainSmote,ytest
...
def trainModel(xtrain: pd.DataFrame, ytrain: pd.Series, hyperparams: Dict[str, Any]) -> RandomForestClassifier:
    ...
    logger.info(f"Training model with {len(xtrain)} samples and {len(ytrain)} target values")
...
def train():
    ...
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        #Save the reference data for monitoring
        ...
        mlflow.sklearn.log_model(sk_model=model, name="model", input_example=xtrain[0:3], signature=create_signature_with_doubles(xtrain))
